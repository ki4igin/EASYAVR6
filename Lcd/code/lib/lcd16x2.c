...
void LcdInit(void)
{
    ...
    /* Иницилизация диспля и перевод его на 4-х битный интерфейс
...
static void LcdSendHfByte(uint8_t hfbyte)
{
    ...
    (выводы данных дисплея подключены к старшим разрядам порта микроконтроллера)
    */
    ...
    // Записываем данные/команду в дисплей
    LCD_PORT &= ~(1 << LCD_E);
}
// End File --------------------------------------------------------------------