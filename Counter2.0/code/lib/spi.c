...
Используется прерывания по завершению обмена
*******************************************************************************/
void SpiInit(void)
{
    /* Инициализация выводов:
    SCK, MOSI, SS и CS - выходы (SS устанавливается как выход чтобы модуль SPI
    случайно не перешел в режим Slave);
    MISO - вход
    */
...
void SpiTxRx(uint8_t* pbuf, uint8_t bufSize)
{
    ...
    // Установка размера и указателя на буфер обмена
    bufSizeSpi = bufSize;
    pbufSpi    = pbuf;
