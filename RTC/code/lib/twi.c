...
typedef enum
{
    TWI_NOINIT = 0,  // TWI не иницилизирован
...
static TwiStatus_t twiStatus = TWI_NOINIT;  // Состаяние модуля TWI
...
void TwiInit(void)
{
    // Установка коэффициента деления для генератора тактового сигнала
...
void TwiWrite(uint8_t slaw, uint8_t regAddr, uint8_t* pbuf, uint8_t bufSize)
{
    ...
    // записи, указатель на буфер для записи, размер буфера для записи    
    twiSlaW    = slaw;
    twiRegAddr = regAddr;
    pbufTwi    = pbuf;
    bufSizeTwi = bufSize;
...
void TwiRead(uint8_t slaw, uint8_t regAddr, uint8_t* pbuf, uint8_t bufSize)
{
    ...
    // Установка адреса ведомого устройства, адреса начального регистра для
    // чтения, указатель на буфер для чтения, размер буфера для чтения    
    twiSlaW    = slaw;
    twiRegAddr = regAddr;
    pbufTwi    = pbuf;
    bufSizeTwi = bufSize;
...
TwiFlags_t TwiTwsi(void)
{
    ...
    switch (statusLine)
    {
        case LINE_BUS_ERROR:
            ...
            break;
        case LINE_START:
            // Передача пакета SLA+W
            TWDR = twiSlaW;
            TWCR &= ~(1 << TWSTA);
            break;
        case LINE_REP_START:
            // Передача пакет SLA+R
            TWDR = twiSlaW + 1;
            TWCR &= ~(1 << TWSTA);
            break;
        ...
        case LINE_TX_DATA_ACK:
            // Взависимости от состояния модуля TWI, переходим к считыванию
        ...
        case LINE_TX_DATA_NACK:
            ...
            // состояние TWI_READY и флага txc
            TWCR |= (1 << TWSTO);
            status   = TWI_READY;
            flag.txc = 1;
            break;
        ...
        case LINE_RX_DATA_NACK:
            // Запись поселеднего принятого байта данных в буфер чтения и
            // завершение передачи - формирование состояния СТОП, установка
            // состояние TWI_READY и флага txc
            *pbufTwi++ = TWDR;
            TWCR |= (1 << TWSTO);
            ...
            break;
        default:
            // Формирование состояния СТОП и установка состояние TWI_READY
            TWCR |= (1 << TWSTO);
            status = TWI_READY;
            break;
    }
    ...
    TWCR |= (1 << TWINT);
    twiStatus = status;
    // Возвращение переменноый с флагами завершения операций
    return flag;
}
// End File --------------------------------------------------------------------